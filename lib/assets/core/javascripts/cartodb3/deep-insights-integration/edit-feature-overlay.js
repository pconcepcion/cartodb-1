...
var EditFeatureOverlay = CoreView.extend({
  ...
  show: function () {
    ...
    this.$el.show();
    // Just after showing the overlay a click event is triggered use a timeout to prevent the overlay from disappearing. 
    setTimeout(function () { $(document).on('click', this.hide); }.bind(this), 10);
  },
